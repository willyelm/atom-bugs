...
class NodeBugs extends EventEmitter {
  ...
  // parseEvaluation (source) {
  //   function getRefs (data, path = '') {
  //     let references = {}
  //     _.forEach(source, (value, name) => {
  //       if (name === 'ref') {
  //         references[value] = path
  //       } else if (_.isObject(value)) {
  //         if (_.isNumber(name)) {
  //           name = `[${name}]`
  //         }
  //         let r = getRefs(value, path ? `${path}.${name}` : name)
  //         if (_.size(r)) {
  //           _.merge(references, r)
  //         }
  //       }
  ...
  //     let refs = getRefs(source)
  //     if (_.size(refs) > 0) {
  //       this.client.reqLookup(_.keys(refs), (e, res) => {
  //         _.forEach(res, (value, ref) => {
  //           _.set(source, refs[ref], value)
  //           let hasRefs = getRefs(value)
  //           console.log(hasRefs)
  //         })
  //       })
